...
class BenchmarkEngine:
    ...
    def run_benchmark(
        ...
        summary = {
            ...
            "algorithm": algorithm,
            "iterations": iterations,
            "avg_db_latency_ms": round(statistics.mean([r.db_latency_ms for r in results]), 3),
            "avg_masking_latency_ms": round(
                statistics.mean([r.masking_latency_ms for r in results]), 3
            ),
            "avg_total_latency_ms": round(statistics.mean(total_latencies), 3),
            "avg_overhead_percent": round(
                statistics.mean([r.overhead_percent for r in results]), 2
            ),
            "avg_cpu_percent": round(statistics.mean([r.cpu_percent for r in results]), 2),
            "avg_ram_mb": round(statistics.mean([r.ram_mb for r in results]), 2),
            "throughput_qps": round(statistics.mean([r.throughput_qps for r in results]), 2),
            "p50_ms": round(total_latencies_sorted[p50_idx], 3),
            "p95_ms": round(
                total_latencies_sorted[min(p95_idx, len(total_latencies_sorted) - 1)], 3
            ),
            "p99_ms": round(
                total_latencies_sorted[min(p99_idx, len(total_latencies_sorted) - 1)], 3
            ),
